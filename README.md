...
**VWAP Execution Engine** · `29,276 lines` · `Solidity, TypeScript` · `Feb 2026`
> Algorithmic large-order execution protocol that slices trades into time-weighted tranches to minimize market impact. The core contract uses the Checks-Effects-Interactions pattern, ReentrancyGuard, and SafeERC20, with bitmask slice tracking (uint256 bit flags) to avoid dynamic array writes on state updates. A SimpleAMM with constant-product pricing (x*y=k) serves as the execution benchmark. Built a TypeScript keeper bot with event-driven slice triggering, a flow bot for realistic load simulation, and a Next.js 14 frontend with wagmi v2 and viem visualizing per-slice fill prices and cumulative VWAP deviation in real time. Deployed on a parallel-execution EVM chain where sub-second time-sliced execution is viable at scale; demonstrated 5-10x slippage reduction versus single-block market execution.
